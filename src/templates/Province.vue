<template>
  <Layout>
    <div class="state-page" v-if="stateMeta && stateDaily && stateHistory">
      <main>
        <h1
          v-html="convertRegion(this.$route.params.province.toUpperCase(), 1)"
        />
...
export default {
  ...
  async mounted() {
    ...
    if (
      this.$store.states &&
      this.$store.states[this.$route.params.province.toLowerCase()]
    ) {
      stateMeta = this.$store.states[this.$route.params.province].stateMeta;
      stateDaily = this.$store.states[this.$route.params.province].stateDaily;
      stateHistory = this.$store.states[this.$route.params.province]
        .stateHistory;
    } else {
